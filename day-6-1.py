...
def __parse_input_file() -> Tuple[list[int], list[int]]:
    with open(INPUT_FILE_NAME) as input_file:
        times = [
            int(stringified_time)
            for stringified_time in input_file.readline().strip().split(":")[1].split()
        ]
        distances = [
            int(stringified_distance)
            for stringified_distance in input_file.readline()
            .strip()
            .split(":")[1]
            .split()
        ]
